  ...
  function numberTest(input) {
    return new Promise(function (resolve, reject) {
      ...
      if (phoneRegexp) {
        ...
        // removeError(phoneNumber.value); // Если номер подходит, то удали ошибку и передай конретное поле
        resolve(input);
      } else {
        console.log("Номер не подходит");
        // addError(phoneNumber.value); // Если номер не подходит, то добавь ошибку и передай конретное поле
        reject(input);
      }
    });
  }

  // async function testValidate(input) {
  //   try {
  //     console.log(input);
  //     numberTest();
  //   } catch (err) {
